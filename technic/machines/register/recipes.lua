...
local have_cg = minetest.get_modpath("hades_craftguide2")
...
function technic.register_recipe_type(typename, origdata)
	...
	if have_cg and hades_craftguide2.register_craft_type and data.output_size == 1 then
		hades_craftguide2.register_craft_type(typename, {
			description = data.description,
			--width = data.input_size,
			--height = 1,
		})
...
local function register_recipe(typename, data)
	...
	if have_cg and technic.recipes[typename].output_size == 1 then
		if hades_craftguide2.register_craft then
