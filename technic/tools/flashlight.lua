...
minetest.register_alias("hades_technic:light_off", "air")

for key, data in pairs(technic.battery_types) do
  technic.register_power_tool("hades_technic:flashlight_"..data.name, 2*data.max_charge)

  minetest.register_tool("hades_technic:flashlight_"..data.name, {
    description = S("Flashlight"),
    _tt_help = S("With "..data.desc.." battery."),
    inventory_image = "technic_flashlight.png",
    stack_max = 1,
    groups = {technic_flashlight = 1},
    wear_represents = "technic_RE_charge",
    on_refill = technic.refill_RE_charge,
    _max_charge = 2*data.max_charge,
  })

  minetest.register_craft({
    output = "hades_technic:flashlight_"..data.name,
    recipe = {
      {"hades_technic:rubber",                "hades_core:glass",   "hades_technic:rubber"},
      {"hades_technic:stainless_steel_ingot", "hades_technic:battery_"..data.name, "hades_technic:stainless_steel_ingot"},
      {"",                              "hades_technic:battery_"..data.name, ""}
    }
  })
end
...
local function check_for_flashlight(player)
	...
	for i = 1, 8 do
    local item_def = hotbar[i]:get_definition();
		if item_def.groups and item_def.groups.technic_flashlight then
			...
			if meta and meta.charge and meta.charge >= 2 then
				if not technic.creative_mode then
					...
					technic.set_RE_wear(hotbar[i], meta.charge, item_def._max_charge)
...
minetest.register_on_leaveplayer(function(player)
	...
	if nodename == "hades_technic:light" then
...
minetest.register_globalstep(function(dtime)
	for i, player in pairs(minetest.get_connected_players()) do
		...
		if was_wielding[player_name] and not flashlight_weared then
			...
			if node and node.name == "hades_technic:light" then
		...
		elseif (player_moved or not was_wielding[player_name]) and flashlight_weared then
			...
			if node and node.name == "air" then
				minetest.set_node(rounded_pos, {name="hades_technic:light"})
			end
...
minetest.register_node("hades_technic:light", {
	drawtype = "glasslike",
