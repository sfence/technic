...
local vacuum_charge_per_object = 200   -- 100   - Capable of picking up 50 objects
local vacuum_range             = 8     -- 8     - Area in which to pick up objects

local S = technic.getter

for key, data in pairs(technic.battery_types) do
  technic.register_power_tool("hades_technic:vacuum", 2*data.max_charge)

  minetest.register_tool("hades_technic:vacuum", {
  ...
  minetest.register_craft({
    output = 'hades_technic:vacuum_'..data.name,
    recipe = {
      {'pipeworks:tube_1',              'pipeworks:filter', 'hades_technic:battery_'..data.name},
      {'pipeworks:tube_1',              'hades_extramaterials:motor',    'hades_technic:battery_'..data.name},
